...
export default async function handler(
  ...
  if (session) {
    ...
    console.log(
      "User Level: ",
      session.user.email == process.env.ADMIN ? "ADMIN" : "USER"
    );
  }
      ...
      case 2:
        switch (method) {
          case "GET":
            ...
            const resultGet = await findAllItemsLimit(
              type[0],
              parseInt(type[1])
            );
      ...
      case 1:
        switch (method) {
          ...
          case "POST":
            ...
            const resultPost = await createItem({
              item: req.body.item,
              type: type[0],
            });
          ...
          case "PUT":
            ...
            const resultPut = await createItem({
              item: req.body.item,
              type: type[0],
            });
          ...
          case "DELETE":
            // Update or create data in your database
            const resultDelete = await deleteItem({
              id: req.body.id,
              type: type,
            });
