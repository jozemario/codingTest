...
async function startScheduler() {
        ...
        const jobs:any[] = []; jobs.push(global.OUTQueue.shift())
        const source:any[] = jobs

        if (jobs.length > 0) {
            

        async.timesLimit(source.length, max_threads_master, (n, next) => {
            ...
            processQueue(source[n])
                ...
                let workerTasks = [].concat.apply([], stdoutArray);
                        ...
                        async.timesLimit(workerTasks.length, max_threads_worker, (n, next) => {
